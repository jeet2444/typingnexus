...
const UserProfile: React.FC = () => {
  ...
  // Simple Chart Mock using CSS heights
  const weeklyProgress = [40, 45, 42, 50, 48, 55, 62];
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {currentUser.plan !== 'Free' ? (
                      <tr className="hover:bg-gray-800/50 transition-colors">
                        <td className="px-6 py-4 font-medium text-gray-300">{currentUser.joined}</td>
                        <td className="px-6 py-4 font-bold text-white">{currentUser.plan} Subscription</td>
                        <td className="px-6 py-4 text-gray-300">₹{currentUser.plan.includes('Yearly') ? '499' : '199'}</td>
                        <td className="px-6 py-4"><span className="px-2 py-1 bg-green-500/10 text-green-400 rounded text-xs font-bold border border-green-500/20">Paid</span></td>
                        <td className="px-6 py-4 text-blue-400 font-bold cursor-pointer hover:text-white transition-colors">Download</td>
                      </tr>
                    ) : (
                      <tr>
