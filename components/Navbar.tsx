...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      ...
      {isAdmin && (
        <div className="bg-black text-white text-xs font-bold px-4 py-1.5 flex justify-between items-center relative z-[60] border-b border-gray-800">
          <div className="flex items-center gap-4">
            <Link to="/" className="flex items-center gap-1.5 hover:text-brand-purple transition-colors">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
              Back to Site
            </Link>
            <span className="text-gray-600">|</span>
            <Link to="/admin" className="hover:text-brand-purple transition-colors flex items-center gap-1">
              <span className="w-3 h-3 flex items-center justify-center border border-gray-500 rounded text-[8px] font-mono">1</span>
              <span>Updates</span>
            </Link>
          </div>

          <div className="flex items-center gap-4">
            <span className="text-gray-400">System Command:</span>
            <span className="text-brand-purple">ROOT ACCESS GRANTED</span>
          </div>
        </div>
      )}

      {/* Main Navbar */}
      <nav className={`sticky top-0 z-50 bg-gray-900/80 backdrop-blur-xl border-b border-gray-800 px-4 sm:px-8 py-3 transition-all shadow-lg shadow-black/20 ${isAdmin ? 'top-[32px]' : ''}`}>
